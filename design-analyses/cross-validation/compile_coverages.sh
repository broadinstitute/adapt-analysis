...
for dir in $(ls -1 . | grep 'tax-'); do
    ...
    for constrainttype in "standard" "relaxed"; do
        ...
        echo -e "active\thighly_active" > tax-${design_name}/coverage-against-test.${constrainttype}.distribution.txt

        # Compute a coverage value for each resampling
        for design_file_active in $(ls -1 tax-${design_name}/designs/$constrainttype/coverages/design-*.active.txt); do
            ...
            echo -e "$topn_mean_active\t$topn_mean_highly_active" >> tax-${design_name}/coverage-against-test.${constrainttype}.distribution.txt
        done
    done
done
